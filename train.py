...
def train_ae(net_params, layer_params_list, train_files, valid_files, min_value, max_value,
    ...
    logger.debug('Starting model definition...')
    ...
    logger.debug('Model successfully compiled.')
    logger.debug('Network parameters: {}'.format(str(net_params)))
    for i, layer_params in enumerate(layer_params_list):
        logger.debug('Layer {} parameters: {}'.format(str(i + 1), str(layer_params)))
...
def classify(ae, train_files, valid_files, min_value, max_value, logger):
    ...
    for layer in ae.layers[:int(len(ae.layers) / 2) + 1]:
        classifier.add(layer)
    for layer in classifier.layers:
        layer.trainable = False
    classifier.add(layers.Dense(2, 'sigmoid', name='dense_cls'))
    classifier.compile('nadam', 'binary_crossentropy', metrics=['accuracy'])
    ...
    for epoch in range(1, 201):
        ...
        if epoch % 10 == 0:
            classifier.save('checkpoints/classifier_checkpoint.h5')
            K.clear_session()
            classifier = models.load_model('checkpoints/classifier_checkpoint.h5')
        for train_file in train_files:
            valid_file = np.random.choice(valid_files)
            logger.debug('Training file: {}'.format(train_file))
            logger.debug('Validation file: {}'.format(valid_file))
        ...
        training_history['aramis_metric'].append(aramis_metric(true_labels_train, predicted_labels_train))
        training_history['val_aramis_metric'].append(aramis_metric(true_labels_valid, predicted_labels_valid))
        for key in history.history:
...
def predict(classifier, test_files):
